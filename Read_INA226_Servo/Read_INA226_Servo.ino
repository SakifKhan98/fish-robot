...
void loop() {
  ...
  float current = ina.getCurrent() / 1000.0;   // in amps (convert mA to A)
  ...
  for (pos = 0; pos <= 170; pos += 1) {  // Sweep from 0 to 180 degrees
  ...
  // delay(100);  // Wait for 1 second before next read (sampling rate)
}
